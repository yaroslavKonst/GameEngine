...
void Menu::LoadResources()
{
	...
	_buttonBackground = _common->video->LoadTexture(buttonBackgroundImage);
}

void Menu::UnloadResources()
{
	_common->video->UnloadTexture(_buttonBackground);
}

void Menu::LoadButtons()
{
	_playButton = new Button(_common->video, _common->textHandler);
	_exitButton = new Button(_common->video, _common->textHandler);
	_continueButton = new Button(_common->video, _common->textHandler);
	_exitToMenuButton = new Button(_common->video, _common->textHandler);

	_playButton->SetText(_common->localizer->Localize("Play"));
	_exitButton->SetText(_common->localizer->Localize("Exit"));
	_continueButton->SetText(_common->localizer->Localize("Continue"));
	_exitToMenuButton->SetText(_common->localizer->Localize("Exit"));
